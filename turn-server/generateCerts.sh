...
openssl req -config "./openssl.cnf" -key "${CA_NAME}.pem" -new -sha256 -subj '/C=NL' -out "${CA_NAME}.csr"
...
openssl req -config "./openssl.cnf" -key "${SERVER_NAME}.pem" -new -sha256 -subj '/C=NL' -out "${SERVER_NAME}.csr"
...
openssl req -config "./openssl.cnf" -key "${CLIENT_NAME}.pem" -new -sha256 -subj '/C=NL' -out "${CLIENT_NAME}.csr"
